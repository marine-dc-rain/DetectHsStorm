...
class StormDetectionTracking:
	...
	def run_detection(self,months_years,args):
		...
		for mm,yy in months_years:
			...
			if ismp:
				pool = mp.Pool(Nb_CPU)
				results = pool.starmap_async(get_storm_by_timestep, [(ds.isel(time=it), cte.levels, cte.Npix_min, cte.amp_thresh, cte.d_thresh_min, cte.d_thresh_max, cte.area_forgotten_ratio, cte.min_area) for it in range(ds.sizes['time'])]).get()
				pool.close()
				r_xr = xr.concat(results,dim='x').sortby('time')
			else:
				results = []
				for it in range(ds.sizes['time']):
					_results = get_storm_by_timestep(ds.isel(time=it), cte.levels, cte.Npix_min, cte.amp_thresh, cte.d_thresh_min, cte.d_thresh_max, cte.area_forgotten_ratio, cte.min_area)
	...
	def init_option(self):
		parser = StormParser(prog="detect_track_storms",
							 usage="%prog [-yyear] [-fyfinal_year] [-mmonth] [-fmfinal_month] [-s'0|1']",
							 description="%(prog) v1.1")
