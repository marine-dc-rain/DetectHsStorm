...
from detection_code.storms_functions_io import alti_read_l2lr, alti_read_l2lr_cci
...
def rename_duplicates(regions, NX):
    ...
    regions_dupl0 = np.zeros_like(map_m180180) - 100.0
    regions_dupl0[:, : np.size(map_180360, 1)] = np.copy(map_180360)

    regions_dupl2 = np.zeros_like(map_m180180) - 100.0
    ...
    regions_orig[(regions_dupl0 > -1) & (BN_inters_0 == False)] = regions_dupl0[
        (regions_dupl0 > -1) & (BN_inters_0 == False)
    ]
...
def get_storm_info_from_savemap(ds, addvarlist=[]):
    ...
    dsn = xr.Dataset(
        {
            'lon_max': (idm.longitude + 180) % 360 - 180,
...
def get_storm_from_model_by_timestep(
    ds1, levels, amp_thresh, min_area, area_forgotten_ratio, plot_output=False, plot_example=False
):
    # --- concat [-180: 360] ----
    ds2 = ds1.copy(deep=True).sel(longitude=slice(None, 0))  # OK for WW3 that starts at -180 but ERA5 ???
    ds2['longitude'] = ds2['longitude'] + 360.0
    ...
    swh_filt0 = spatial_filter(dsTot['hs'].data, 0.5, 4.0, 4.0)
    ...
    for ilev, lev in enumerate(levels):
        ...
        if plot_example:
            if ilev == level_selec:
                ...
                ax = axs[0, 1]
                im = ax.pcolormesh(
                    dsTot.longitude, dsTot.latitude, regions_new, cmap='jet', vmin=-1, vmax=regions_new.max()
                )
                plt.colorbar(im, ax=ax)
                ax.set_title('renamed regions ')
                ax = axs[1, 0]
                im = ax.pcolormesh(
                    dsTot.longitude, dsTot.latitude, regions_interOnly, cmap='jet', vmin=-1, vmax=regions_new.max()
                )
                plt.colorbar(im, ax=ax)
                ...
                ax = axs[1, 1]
                im = ax.pcolormesh(
                    dsTot.longitude, dsTot.latitude, regions_only_big_area, cmap='jet', vmin=-1, vmax=regions_new.max()
                )
                plt.colorbar(im, ax=ax)
                ax.set_title('unique regions with only big area')
                ax = axs[2, 0]
                im = ax.pcolormesh(
                    dsTot.longitude, dsTot.latitude, (to_save_intersec > -1), cmap='grey', vmin=0, vmax=1
                )
                plt.colorbar(im, ax=ax)
                ...
                im = ax.pcolormesh(
                    dsTot.longitude,
                    dsTot.latitude,
                    (to_save_intersec < -1) * (regions_new > -1),  # +1)*(regions_new >-1),
                    cmap='grey',
                    vmin=0,
                    vmax=1,
                )
        ...
        for iir, ir in enumerate(uregions):
            ...
            u_old_regions_in0 = np.unique(
                (regions_old[(to_save_intersec < -2) & (regionNB) & (regions_old > -1)])
            )  # regions that are not saved
    ...
    g = ds3.where(ds3.regions > -1.0, np.nan).groupby('regions')
    res = g.map(get_storm_info_from_savemap, addvarlist=cte.addvarlist).swap_dims({'regions': 'x'})
        ...
        if len(peakind) > 0:
            ...
            if len(peakind) > 1:
                ...
                for ipic, pic in enumerate(peakind):
                    ...
                    not_inside_bases = (props['left_ips'][ipic] < props['left_ips'][inddif]) | (
                        props['right_ips'][ipic] > props['right_ips'][inddif]
                    )
                    not_inside_ips = (props['left_bases'][ipic] < props['left_bases'][inddif]) | (
                        props['right_bases'][ipic] > props['right_bases'][inddif]
                    )
                    ...
                    if np.all(not_inside_bases & not_inside_ips):  # not included in any of others => save
                        ...
                        if output == 'stats_histo':
                            ...
                            H_bases0, _ = np.histogram(
                                swh_denoised.isel(time=slice(props['left_bases'][ipic], props['right_bases'][ipic])),
                                Hs_bins,
                            )
                            H_ips.append(H_ips0)
                ...
                if output == 'stats_histo':
                    H_ips0, _ = np.histogram(swh_denoised.isel(time=slice(i1, i2)), Hs_bins)
                    H_bases0, _ = np.histogram(
                        swh_denoised.isel(time=slice(props['left_bases'][0], props['right_bases'][0])), Hs_bins
                    )
                    H_ips.append(H_ips0)
                    H_bases.append(H_bases0)
            ...
            if 'stats' in output:
                ...
                above_thresh = xr.Dataset(dict(Hs_max=Hs_max, ips_xr=ips_xr))
